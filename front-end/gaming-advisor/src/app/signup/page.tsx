...
const handleSubmit = async (event: React.FormEvent) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // On vérifie si data.detail est bien un tableau d'erreurs
      if (data.detail && Array.isArray(data.detail)) {
        // On extrait le message de chaque objet d'erreur et on les assemble
          errorMessage = data.detail.map((err: { msg: string }) => err.msg).join('. ');
      } else if (data.detail) {
        // Si c'est un simple message, on l'utilise
        errorMessage = data.detail;
      }

      // On lance la nouvelle erreur avec notre message bien formaté
